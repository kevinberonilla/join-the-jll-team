<?php 
/*
Template Name: Admin Reports Export
*/
//get_header('admin'); 
...
//echo '<pre>'; print_r($result->rows); echo '</pre>';
...
echo "<pre>"; print_r($idsRow); echo "</pre>";
...
foreach ( $result->rows as $key => $row )
{
	if ( !in_array(md5($row[0] . $row[2] . date('m/d/Y h:i a', strtotime( $row[3]." ".$row[4].":".$row[5] ))), $idsRow) )
	{
		...
		echo '<pre>'; print_r($set_entry_parameters_Usage); echo '</pre>';
		//echo '<pre>'; print_r($set_entry_result_Usage); echo '</pre>';
	}
}
